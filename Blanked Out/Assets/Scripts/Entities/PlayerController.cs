...
public class PlayerController : MonoBehaviour
{
    ...
    [Range(0, 1)][SerializeField] private float m_AirSpeedMultiplier = 0.1f;   // Speed multiplier when airborne
    ...
    public void Move(float move, bool crouch, bool jump)
    {
        ...
        if (m_Grounded || m_AirControl)
        {
            ...
            // Adjust movement speed based on whether the player is grounded
            float adjustedMove = move;
            if (!m_Grounded)
            {
                adjustedMove *= m_AirSpeedMultiplier;
            }

            // Move the character by finding the target velocity
            Vector3 targetVelocity = new Vector2(adjustedMove * 10f, m_Rigidbody2D.linearVelocity.y);
    ...
    // Visual debugging for GroundCheck and CeilingCheck
